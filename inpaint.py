...
from inpainting.im2Video import im2Video
...
def inpaint(args):
    ...
    ########## DAVIS
    DAVIS_ROOT = './results'
    # DTset = DAVIS(args, DAVIS_ROOT, mask_dilation=args.mask_dilation, size=(opt.crop_size, opt.crop_size))
    DTset = DAVIS(args, DAVIS_ROOT, size=(opt.crop_size, opt.crop_size))
    ...
    with torch.no_grad():
        for seq, (inputs, masks, info) in enumerate(DTloader):
            ...
            if opt.save_video:
                ...
                video_path = args.outputFolder
                if not os.path.exists(video_path):
                    os.makedirs(video_path)

                im2Video(video_path, 'png', 'inpainting', 'mp4', [1280, 720])
