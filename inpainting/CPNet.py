...
def get_arguments():
    ...
    parser.add_argument("-D", type=str, default='./') # dataset path
...
with open("../tmp.txt", 'r') as f:
    for line in f:
        line = line.rstrip('\n')
        DATA_ROOT = line.split(' ')[0]
        videoName = line.split(' ')[1]
        break

args.videoFolder = DATA_ROOT
...
Pset = DAVIS_Test(root=DATA_ROOT, name=videoName, size=[240, 424])
...
for i, V in tqdm.tqdm(enumerate(Trainloader)):
    ...
    for t in range(2): # forward : 0, backward : 1
        ...
        for f in index:
            ...
            with torch.no_grad():
                comp = model(idx, f, ridx, rfeats[:,:,ridx], frames[:,:,ridx], masks[:,:,ridx], frames[:,:,f], masks[:,:,f], GTs[:,:,f], args)
                idx += 1
            ...
            #     canvas.save(os.path.join(save_path, 'f{}.png'.format(f)))

    # vid_path = os.path.join('./test/', '{}.mp4'.format(seq_name))
    # frame_path = os.path.join('./test/', seq_name, 'f%d.png')
    # os.system('ffmpeg -framerate 30 -i {} {}  -nostats -loglevel 0 -y'.format(frame_path, vid_path))
    # print('----------------------------------------------------------')
