...
class DAVIS_Test(data.Dataset):
    def __init__(self, root, name, imset='2016/trainval.txt', resolution='480p', size=None):
        self.root = root
        # self.mask_dir = os.path.join(root, 'Annotations', resolution)
        # self.image_dir = os.path.join(root, 'JPEGImages', resolution)
        # _imset_dir = os.path.join(root, 'ImageSets')
        # _imset_f = os.path.join(_imset_dir, imset)
        self.mask_dir = os.path.join(root, 'masks/siamMasks')
        self.image_dir = os.path.join(root, 'frames')
    ...
    def __getitem__(self, index):
        ...
        for i, f in enumerate(idxs):
            ...
            raw_frame = cv2.imread(img_file)
            raw_frame = cv2.cvtColor(raw_frame, cv2.COLOR_BGR2RGB) / 255.
            N_frames[i] = cv2.resize(raw_frame, dsize=(W, H), interpolation=cv2.INTER_CUBIC)

            mask_file = os.path.join(self.mask_dir, '{:05d}.png'.format(f))  
            raw_mask = np.array(Image.open(mask_file).convert('P'), dtype=np.uint8)
            raw_mask = (raw_mask > 0.5).astype(np.uint8)
            raw_mask = cv2.resize(raw_mask, dsize=(W, H), interpolation=cv2.INTER_CUBIC)
