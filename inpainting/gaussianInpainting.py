...
def gaussianInpainting(txtFolder, output_folder, curFolder, curFrame):
    ...
    total_weight = np.zeros(target_frame.shape, dtype = 'float64')
    total_visible = np.zeros(target_frame.shape, dtype = 'float64')

    curFrame = "%05d" % curFrame
    curIndex = bisect_left(afNames, tfName.replace('target', curFrame))

    for i, (afName, amName) in enumerate(zip(afNames, amNames)):
        frame = cv2.imread(afName)
        mask = cv2.imread(amName)
        distance = abs(i-curIndex)
        if i >= curIndex:
            distance += 1
        weight = gaussian(distance, 0, 4)
        visible_mask = (mask > 128) * (target_mask > 128) * weight
