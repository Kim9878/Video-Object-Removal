...
class CM_Module(nn.Module):
    ...
    def forward(self, values, tvmap, rvmaps):
        ...
        for r in range(T):
            rvmap_t = (F.interpolate(rvmaps[:,:,r], size=(H, W), mode='bilinear', align_corners=False)>0.5).float()
            # vmap: visibility map
...
class CPNet(nn.Module):
    ...
    def inpainting(self, currentIdx, currentFrame, refFrameIdx, rfeats, rframes, rholes, frame, hole, gt, args): 
        ...
        save_path = os.path.join(args.videoFolder + '/alignedFrame/' + str(currentIdx) + '/')
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        fp = open(save_path + 'aligned.txt', "w")

        fp.write(str(currentFrame) + '\n')
        
        for r in range(num_r):
            theta_rt = self.A_Regressor(tfeat, rfeats[:,:,r])
            grid_rt = F.affine_grid(theta_rt, frame.size())

            py_list = theta_rt.tolist()
            t0 = py_list[0][0][0]
            t1 = py_list[0][0][1]
            t2 = py_list[0][0][2]
            t3 = py_list[0][1][0]
            ...
            fp.write(str(refFrameIdx[r]) + ' ' + str(t0) + ' ' + str(t1) + ' ' + str(t2) + ' ' + str(t3) + ' ' + str(t4) + ' ' + str(t5) + '\n')
